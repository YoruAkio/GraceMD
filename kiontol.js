...
global.api = async (name, options = {}) =>
    new (await import('./lib/api.js')).default(name, options);
...
const pairingCode =
    !!config.options.pairingNumber || process.argv.includes('--pairing-code');
...
async function start() {
    ...
    const { state, saveCreds } = await useMultiFileAuthState(
        `./${config.options.sessionName}`,
    );
    const msgRetryCounterCache = new NodeCache(); // for retry message, "waiting message"

    const kiontol = baileys.default({
    ...
    store.bind(kiontol.ev);

    // push update name to store.contacts
    kiontol.ev.on('contacts.update', update => {
    ...
    // bind extra client
    await Client({ kiontol, store });

    // login use pairing code
    // source code https://github.com/WhiskeySockets/Baileys/blob/master/Example/example.ts#L61
    if (pairingCode && !kiontol.authState.creds.registered) {
        if (useMobile)
        ...
        setTimeout(async () => {
            let code = await kiontol.requestPairingCode(phoneNumber);
    ...
    if (useMobile && !kiontol.authState.creds.registered) {
        const { registration } = kiontol.authState.creds || {
            registration: {},
        };
        ...
        async function enterCode() {
            try {
                const code = await question(
                    chalk.bgBlack(
                        chalk.greenBright(`Please Enter Your OTP Code : `),
                    ),
                );
                const response = await kiontol.register(
                    code
                        .replace(/[^0-9]/g, '')
                        .trim()
                        .toLowerCase(),
        ...
        async function enterCaptcha() {
            ...
            const code = await question(
                chalk.bgBlack(
                    chalk.greenBright(`Please Enter Your Captcha Code : `),
                ),
            );
        ...
        async function askOTP() {
            ...
            try {
                await kiontol.requestRegistrationCode(registration);
                await enterCode();
            } catch (e) {
                console.error(
    ...
    kiontol.ev.on('connection.update', async update => {
    ...
    // write session
    kiontol.ev.on('creds.update', saveCreds);

    // messages
    kiontol.ev.on('messages.upsert', async message => {
        if (!message.messages) return;
        const m = await Serialize(kiontol, message.messages[0]);
        await (
            await import(`./event/message.js?v=${Date.now()}`)
        ).default(kiontol, m, message);
    });

    // group participants update
    kiontol.ev.on('group-participants.update', async message => {
        await (
            await import(`./event/group-participants.js?v=${Date.now()}`)
        ).default(kiontol, message);
    });

    // group update
    kiontol.ev.on('groups.update', async update => {
        await (
            await import(`./event/group-update.js?v=${Date.now()}`)
        ).default(kiontol, update);
    });

    // auto reject call when user call
    kiontol.ev.on('call', async json => {
        if (config.options.antiCall) {
            for (const id of json) {
                if (id.status === 'offer') {
                    let msg = await kiontol.sendMessage(id.from, {
                    ...
                    kiontol.sendContact(id.from, config.options.owner, msg);
                    await kiontol.rejectCall(id.id, id.from);
                }
            }
        }
    });
