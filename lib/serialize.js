...
// jangan diperjualbelikan dalam keadaan masih ori kiontol. minimal tambah 5-8 command dulu
...
export function Client({ kiontol, store }) {
    ...
    const client = Object.defineProperties(kiontol, {
        getContentType: {
            value(content) {
                if (content) {
        ...
        getName: {
            value(jid) {
                let id = kiontol.decodeJid(jid);
                let v;
                if (id?.endsWith('@g.us'))
                    return new Promise(async resolve => {
                        v =
                            kiontol.contacts[id] ||
                            kiontol.messages['status@broadcast']?.array?.find(
                                a => a?.key?.participant === id,
                            );
                        if (!(v.name || v.subject))
                            v = kiontol.groupMetadata[id] || {};
                        resolve(
                            v?.name ||
                                v?.subject ||
                                v?.pushName ||
                    ...
                    v =
                        id === '0@s.whatsapp.net'
                            ...
                            : id === kiontol.decodeJid(kiontol?.user?.id)
                            ? kiontol.user
                            : kiontol.contacts[id] || {};
                return (
                    v?.name ||
        ...
        sendContact: {
            async value(jid, number, quoted, options = {}) {
                ...
                for (let v of number) {
                    list.push({
                        displayName: await kiontol.getName(v),
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await kiontol.getName(
                            v + '@s.whatsapp.net',
                        )}\nFN:${await kiontol.getName(
                            v + '@s.whatsapp.net',
                ...
                return kiontol.sendMessage(
                    jid,
                    {
                        ...
                        mentions: quoted?.participant
                            ? [kiontol.decodeJid(quoted?.participant)]
                            : [kiontol.decodeJid(kiontol?.user?.id)],
                        ...options,
                    },
                    { quoted, ...options },
                );
        ...
        downloadMediaMessage: {
            async value(message, filename) {
                ...
                if (
                    'thumbnailDirectPath' in message.msg &&
                    !('url' in message.msg)
                ) {
                    message = {
        ...
        sendMedia: {
            async value(jid, url, quoted = '', options = {}) {
                ...
                if (size > 45000000)
                    data = {
                        ...
                        fileName: options?.fileName
                            ? options.fileName
                            : `${kiontol.user?.name} (${new Date()}).${ext}`,
                        ...options,
                    };
                ...
                let msg = await kiontol.sendMessage(jid, data, {
                    quoted,
                    ...options,
                });
                return msg;
        ...
        cMod: {
            value(
                ...
                copy,
                text = '',
                sender = kiontol.user.id,
                options = {},
            ) {
                let mtype = kiontol.getContentType(copy.message);
                let content = copy.message[mtype];
                if (typeof content === 'string')
                    copy.message[mtype] = text || content;
                else if (content.caption)
                    content.caption = text || content.text;
                ...
                copy.key.fromMe = areJidsSameUser(sender, kiontol.user.id);
        ...
        sendPoll: {
            async value(chatId, name, values, options = {}) {
                let selectableCount = options?.selectableCount
                    ...
                    : 1;
                return await kiontol.sendMessage(
                    chatId,
                    {
                        poll: {
                            name,
            ...
            async value(jid, media, type = 'full') {
                ...
                if (/full/i.test(type)) {
                    ...
                    await kiontol.query({
                        tag: 'iq',
                        attrs: {
                            to: await kiontol.decodeJid(jid),
                            type: 'set',
        ...
        sendGroupV4Invite: {
            async value(
                ...
                const media = await prepareWAMessageMedia(
                    ...
                    { upload: kiontol.waUploadToServer },
                );
                const message = proto.Message.fromObject({
                    ...
                    groupName,
                    jpegThumbnail:
                        media.imageMessage?.jpegThumbnail || jpegThumbnail,
                    caption,
                });

                const m = generateWAMessageFromContent(jid, message, {
                    userJid: kiontol.user?.id,
                });
                await kiontol.relayMessage(jid, m.message, {
                    messageId: m.key.id,
                });
...
export async function Serialize(kiontol, msg) {
    const m = {};
    const botNumber = kiontol.decodeJid(kiontol.user?.id);
    ...
    if (msg.key) {
        m.key = msg.key;
        m.from = kiontol.decodeJid(m.key.remoteJid);
        ...
        m.sender = kiontol.decodeJid(
            m.fromMe ? kiontol.user.id : m.isGroup ? m.participant : m.from,
        );
    }
    ...
    if (m.isGroup) {
        m.metadata = await kiontol.groupMetadata(m.from);
    ...
    if (m.message) {
        m.type = kiontol.getContentType(m.message) || Object.keys(m.message)[0];
        ...
        m.args =
            m.body
                .trim()
                .replace(
                    new RegExp('^' + Function.escapeRegExp(m.prefix), 'i'),
                    '',
                )
                .replace(m.command, '')
                .split(/ +/)
            ...
            if (
                ...
                if (
                    ...
                    return kiontol.sendMessage(
                        ...
                        {
                            text,
                            mentions: [m.sender, ...kiontol.parseMention(text)],
                            ...options,
                        },
                        {
                            quoted,
                    ...
                    return kiontol.sendMedia(m.from, data.data, quoted, {
                        ephemeralExpiration: m.expiration,
                        ...options,
                    });
                }
        ...
        m.download = filepath => {
            if (filepath) return kiontol.downloadMediaMessage(m, filepath);
            else return kiontol.downloadMediaMessage(m);
        };
    }
    ...
    if (m.msg?.contextInfo?.quotedMessage) {
        ...
        if (m.quoted.message) {
            m.quoted.type =
                kiontol.getContentType(m.quoted.message) ||
            ...
            m.quoted.key = {
                ...
                participant: m.msg?.contextInfo?.remoteJid?.endsWith('g.us')
                    ? kiontol.decodeJid(m.msg?.contextInfo?.participant)
                    : false,
            ...
            m.quoted.sender = kiontol.decodeJid(
            ...
            if (m.quoted.isGroup) {
                m.quoted.metadata = await kiontol.groupMetadata(m.quoted.from);
            ...
            m.quoted.args =
                m.quoted.body
                    ...
                    .replace(
                        new RegExp(
                            '^' + Function.escapeRegExp(m.quoted.prefix),
                            'i',
                        ),
                        '',
                    )
            ...
            m.quoted.download = filepath => {
                if (filepath)
                    return kiontol.downloadMediaMessage(m.quoted, filepath);
                else return kiontol.downloadMediaMessage(m.quoted);
            };
        }
    }
